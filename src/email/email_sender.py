...
def get_email_signature(instance_name: Optional[str] = None) -> tuple[str, str]:
    """
    Build a standard email signature with the website URL.

    Returns:
        Tuple of (plain_text_signature, html_signature).
    """
    name = instance_name or settings.instance_name
    url = settings.web_base_url or ""

    if url:
        plain_sig = f"---\n{name}\n{url}"
        html_sig = (
            f'<div style="border-top: 1px solid #D5C9B8; padding-top: 12px; margin-top: 20px; '
            f'font-size: 13px; color: #8C8279;">'
            f"<strong>{name}</strong><br>"
            f'<a href="{url}" style="color: #8C8279;">{url}</a>'
            f"</div>"
        )
    else:
        plain_sig = f"---\n{name}"
...
def load_custom_footer(
    ...
    url_line_plain = f"\n{url}" if url else ""
    url_line_html = (
        f'\n        <p><a href="{url}" style="color: #8C8279;">{url}</a></p>'
        if url
        else ""
    )
    plain_footer = f"\n\n---\nThis response was generated by {instance_name}.{org_line_plain}\nIf you have follow-up questions, simply reply to this email.{url_line_plain}{feedback_plain}"
    html_footer = f"""<div class="footer">
        <p>This response was generated by <strong>{instance_name}</strong>.</p>{org_line_html}
        <p>If you have follow-up questions, simply reply to this email.</p>{url_line_html}
    </div>{feedback_html}"""
...
def format_welcome_email(
    ...
    sections_plain.append(
        ...
        + (f"\n{web_base_url}" if web_base_url else "")
    )
        ...
        <table cellpadding="0" cellspacing="0" border="0">
            <tr>
                ...
                <td style="vertical-align: middle;">
                    ...
                    <span style="font-size: 12px; color: #8C8279;">{signature_subtitle}</span>{'<br><a href="' + web_base_url + '" style="font-size: 12px; color: #8C8279;">' + web_base_url + '</a>' if web_base_url else ''}
                </td>
            </tr>
        </table>
    </div>
